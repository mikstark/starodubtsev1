...
else	// для вывода всего остального
{
	...
	for (int i = 0; i < N.cols; ++i)	// выводим первую строку матрицы/вектора
	{
		...
		{
			cout.precision(4);	// установка числа значащих знаков после запятой
			cout.setf(ios::fixed);	// переход к режиму числа с фиксированной точкой
			elem = to_string(N.matrix[i]).length();
...
// произведение Адамара с двумя параметрами
...
for (int i = 0; i < rows; i++) 
{
	for (int j = 0; j < cols; j++) 
	{
		if (in.peek() == EOF) throw SizeError(string("Matrix in has different dimensions from those that are given"));
